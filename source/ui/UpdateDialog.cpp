...
// 冷蓝白强调色：与 Pupon 界面高光 / 珍珠辉光保持一致
constexpr auto kAccentColour = 0xFFBFD4FF;
...
// 构造
// ==========================================================
UpdateDialog::UpdateDialog(const pupon::network::UpdateInfo& info,
...
// ==========================================================
// paint：暗色玻璃背景 + 顶部 hairline + 标题栏 + 内容区 + 按钮行
// ==========================================================
void UpdateDialog::paint(juce::Graphics& g) {
  const auto dlg = getLocalBounds();

  // 暗色渐变背景（与编辑器背景 0xFF121214 同源）
  juce::ColourGradient bg(juce::Colour(0xFF1C1C21), 0.0f, 0.0f,
                          juce::Colour(0xFF0E0E12), 0.0f,
  ...
  g.setFont(juce::Font(14.0f, juce::Font::bold));
  g.drawText("A new version of Pupon is available!",
  ...
  for (int i = 0; i < numBtns; ++i) {
    ...
    juce::Colour fill = juce::Colours::black.withAlpha(0.55f);
    if (pressedBtn_ == i) {
      fill = accent().withAlpha(0.45f);
    ...
    g.setColour(isDownload ? accent() : juce::Colours::white.withAlpha(0.80f));
    g.drawRoundedRectangle(btnRect.toFloat(), 4.0f, 1.0f);

    g.setColour(isDownload ? accent()
